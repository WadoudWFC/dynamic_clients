            ...
            CancellationToken cancellationToken)
        {
            try
            {
                ...
                var searchQuery = new SearchDossierQuery
                {
                    ...
                    ApplyFilter = true, // Apply role-based filtering
                    Take = request.Take ?? 50,
                    Skip = request.Skip ?? 0,
                    Order = request.Order ?? "desc",
                    Field = request.Field ?? "date_created",
                    // Empty search criteria to get all dossiers
                    Code = null,
                ...
                if (!result.IsSuccess)
                {
                    _logger.LogError("Failed to retrieve dossiers for user {0}: {1}",
