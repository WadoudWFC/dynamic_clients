...
namespace MultipleHttpClient.Application.Standard_User.Dossier.Handlers
{
    public class GetMyDossiersDetailedQueryHandler : IRequestHandler<GetMyDossiersDetailedQuery, Result<MyDossiersDetailedResponse>>
    {
        ...
        public GetMyDossiersDetailedQueryHandler(
            IMediator mediator,
            ILogger<GetMyDossiersDetailedQueryHandler> logger)
        {
            _mediator = mediator;
            _logger = logger;
        }

        public async Task<Result<MyDossiersDetailedResponse>> Handle(
            GetMyDossiersDetailedQuery request,
            CancellationToken cancellationToken)
        {
            try
            {
                ...
                _logger.LogInformation("Successfully generated detailed dossier report for user {UserId}", request.UserId);
                return Result<MyDossiersDetailedResponse>.Success(response);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error generating detailed dossier report for user {UserId}", request.UserId);
                return Result<MyDossiersDetailedResponse>.Failure(
                    new Error(Constants.DossierFail, "Failed to generate detailed report"));
            }
        }
